...
public class HttpPortal {
	...
	private static final String GEONAME_URL = "http://api.geonames.org/search?q=" + INFO_LOC + "&maxRows=1&lang="
			+ INFO_LANG + "&style=full&username=Macadoshis";
	 ...
	 */
	public static Location sendGetRequest(String pInfoLoc, String pLang, RequestNotFound requestNotFound)
		...
		try {
			...
			log.log(Level.WARNING, exc.getMsg(), exc);
			throw exc;
		} catch (Exception exc) {
			log.log(Level.SEVERE, exc.getLocalizedMessage(), exc);
	...
	private static Location getLocationValues(InputStreamReader pXmlContent) throws LocationException, Exception {
		...
		try {
			...
			if (nlRegionName != null && nlRegionName.item(0) != null && nlRegionName.item(0).getFirstChild() != null) {
			...
			if (nlCountryName != null && nlCountryName.item(0) != null && nlCountryName.item(0).getFirstChild() != null) {
